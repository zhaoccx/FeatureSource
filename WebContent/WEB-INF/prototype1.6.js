		...
		Function.prototype,
		{
			...
			bind : function() {
				if (arguments.length < 2
					...
					return this;
				var __method = this, args = $A(arguments), object = args
						.shift();
				return function() {
			...
			delay : function() {
				var __method = this, args = $A(arguments), timeout = args
						.shift() * 1000;
			...
			methodize : function() {
				...
				return this._methodized = function() {
					return __method.apply(null, [ this ]
							.concat($A(arguments)));
				};
			}
		});
...
Object.extend(String.prototype, {
	...
	unescapeHTML : function() {
		...
		return div.childNodes[0] ? (div.childNodes.length > 1 ? $A(
				div.childNodes).inject('', function(memo, node) {
	...
	camelize : function() {
		...
		var camelized = this.charAt(0) == '-' ? parts[0].charAt(0)
				.toUpperCase()
				...
				(function() {
					...
					return {
						initialize : function(object) {
							this._object = Object.isHash(object) ? object
									.toObject() : Object.clone(object);
						},

						_each : function(iterator) {
							for ( var key in this._object) {
								var value = this._object[key], pair = [ key,
										value ];
								pair.key = key;
								pair.value = value;
								iterator(pair);
						...
						update : function(object) {
							return new Hash(object).inject(this, function(
									result, pair) {
						...
						if (params = Object.toQueryString(params)) {
							...
							if (this.method == 'get')
								this.url += (this.url.include('?') ? '&' : '?')
										+ params;
							else if (/Konqueror|Safari|KHTML/
...
Element.Methods = {
	...
	insert : function(element, insertions) {
		...
		for ( var position in insertions) {
			...
			tagName = ((position == 'before' || position == 'after') ? element.parentNode
	...
	siblings : function(element) {
		element = $(element);
		return element.previousSiblings().reverse().concat(
	...
	up : function(element, expression, index) {
		...
		return Object.isNumber(expression) ? ancestors[expression] : Selector
				.findElement(ancestors, expression, index);
	},
	...
	setStyle : function(element, styles) {
		...
		if (Object.isString(styles)) {
			...
			return styles.include('opacity') ? element.setOpacity(styles
		...
		for ( var property in styles)
			if (property == 'opacity')
				...
				elementStyle[(property == 'float' || property == 'cssFloat') ? (Object
...
if (Prototype.Browser.Opera) {
	...
	Element.Methods.readAttribute = Element.Methods.readAttribute
			.wrap(function(proceed, element, attribute) {
				if (attribute === 'title')
	...
	$w('positionedOffset viewportOffset')
			.each(
					function(method) {
						Element.Methods[method] = Element.Methods[method]
								.wrap(function(proceed, element) {
									element = $(element);
									try {
										element.offsetParent
									...
									if (position !== 'static')
										return proceed(element);
									// Trigger hasLayout on the offset parent so
									// that IE6 reports
									// accurate offsetTop and offsetLeft values
									// for position: fixed.
	...
	$w(
			...
			.each(
					function(attr) {
						Element._attributeTranslations.write.names[attr
								.toLowerCase()] = attr;
	...
	Element.Methods.update = function(element, content) {
		...
		if (tagName in Element._insertionTranslations.tags) {
			...
			Element._getContentFromAnonymousElement(tagName,
					content.stripScripts()).each(function(node) {
...
/*
 * Portions of the Selector class are derived from Jack Slocum's DomQuery, part of YUI-Ext version 0.40, distributed under the terms of an MIT-style license. Please see http://www.yui-ext.com/ for
 * more information.
 */
...
var Selector = Class
		.create({
			...
			shouldUseSelectorsAPI : function() {
				...
				// Make sure the browser treats the selector as valid. Test on
				// an
					...
					for ( var i in ps) {
						...
						if (m = e.match(p)) {
							...
							if (as[i]) {
								...
								return this.findElements(document).include(
										element);
							}
						}
					}
				...
				Selector,
				{
					...
					xpath : {
						...
						attr : function(m) {
							m[1] = m[1].toLowerCase();
							m[3] = m[5] || m[6];
							return new Template(Selector.xpath.operators[m[2]])
									.evaluate(m);
						},
						pseudo : function(m) {
							...
							if (Object.isFunction(h))
								return h(m);
							return new Template(Selector.xpath.pseudos[m[1]])
									.evaluate(m);
						},
						operators : {
							'=' : "[@#{1}='#{3}']",
					...
					assertions : {
						tagName : function(element, matches) {
							return matches[1].toUpperCase() == element.tagName
									.toUpperCase();
						},
						...
						attr : function(element, matches) {
							var nodeValue = Element.readAttribute(element,
									matches[1]);
							return nodeValue
					...
					handlers : {
						...
						// mark each child node with its position (for nth
						// calls)
						...
						tagName : function(nodes, root, tagName, combinator) {
							...
							if (nodes) {
								if (combinator) {
									// fastlane for ordinary descendant
									// combinators
						...
						id : function(nodes, root, id, combinator) {
							...
							if (nodes) {
								if (combinator) {
									if (combinator == 'child') {
										...
										for (var i = 0, node; node = nodes[i]; i++)
											if (Element.descendantOf(
													targetNode, node))
												return [ targetNode ];
									} else if (combinator == 'adjacent') {
										for (var i = 0, node; node = nodes[i]; i++)
											if (Selector.handlers
													.previousElementSibling(targetNode) == node)
												return [ targetNode ];
									} else
							...
							return (targetNode && Element.descendantOf(
						...
						className : function(nodes, root, className, combinator) {
							if (nodes && combinator)
								nodes = this[combinator](nodes);
							return Selector.handlers.byClassName(nodes, root,
									className);
						},
						...
						attr : function(nodes, root, attr, value, operator,
							...
							for (var i = 0, node; node = nodes[i]; i++) {
								var nodeValue = Element.readAttribute(node,
										attr);
								if (nodeValue === null)
									continue;
					...
					pseudos : {
						...
						'first-of-type' : function(nodes, formula, root) {
							return Selector.pseudos.nth(nodes, "1", root,
									false, true);
						},
						'last-of-type' : function(nodes, formula, root) {
							return Selector.pseudos.nth(nodes, "1", root, true,
									true);
						},
						'only-of-type' : function(nodes, formula, root) {
							var p = Selector.pseudos;
							return p['last-of-type'](p['first-of-type'](nodes,
									formula, root), formula, root);
						},
						...
						nth : function(nodes, formula, root, reverse, ofType) {
							...
							if (formula.match(/^\d+$/)) { // just a number
								...
								var indices = Selector.pseudos.getIndices(a, b,
										nodes.length);
						...
						'not' : function(nodes, selector, root) {
							...
							var exclusions = new Selector(selector)
									.findElements(root);
							h.mark(exclusions);
							for (var i = 0, results = [], node; node = nodes[i]; i++)
						...
						function(result, element) {
							if (!element.disabled && element.name) {
								...
								if (value != null
									...
									if (key in result) {
										// a key is already present; construct
										// an array of values
...
Form.Methods = {
	...
	findFirstElement : function(form) {
		...
		return firstByIndex ? firstByIndex : elements.find(function(element) {
			return [ 'input', 'select', 'textarea' ].include(element.tagName
					.toLowerCase());
		});
	},
...
Object.extend(Event, (function() {
	...
	function destroyWrapper(id, eventName, handler) {
		...
		c[eventName] = c[eventName]
				.without(findWrapper(id, eventName, handler));
	}
